(function ( $ ) {
    ...
    function renderDays(_){
        ...
        // اینجا تغییر کرده => به جای اینکه روز رو از 1 شروع کنه از 0 شروع می کنه
        ...
        for(var i = 0 ; i <= maxDay ; i++){
            ...
            let CurrentDate = settings.shYear + "-" + settings.shMonth + "-" + i;
            ...
            if (checkMaxDate(settings.shYear, settings.shMonth, i) || checkMinDate(settings.shYear, settings.shMonth, i)) {
                ...
                $.tmplMustache(TEMPLATE.days, { day: i, pick: (settings.pick == "day") ? "pick" : "", datadate: CurrentDate , today:"today" , select : ""}).appendTo($(s.datePickerPlotArea+" "+s.dayView+" "+ s.tableMonthGrid+ " tr[data-week='"+week+"']",_));
            } else {
                $.tmplMustache(TEMPLATE.days, { day: i, pick: (settings.pick == "day") ? "pick" : "", datadate: CurrentDate ,today: "", select: (settings.pshYear == settings.shYear && settings.pshMonth == settings.shMonth && parseInt(settings.pshDay) == i) ? "select" : "" }).appendTo($(s.datePickerPlotArea + " " + s.dayView + " " + s.tableMonthGrid + " tr[data-week='" + week + "']", _));
            }
            first_day++;
        }
    }
    ...
    function initEvents(e){
        ...
        $(s.datePickerPlotArea + " " + s.navigator + " " + ".nav-content", e).bind("click", function () {
            ...
            // برای اینکه کاربر فقط میتواند در ماه جاری نوبت های خود را تایین کند
    ...
    var TEMPLATE  = {
        ...
        "</div>",
        navigator: //"<span class='nav-right'><i style='font-weight: bold;font-size: 9pt;' class='fas fa-arrow-right text-primary' ></i> ماه قبل</span>" +
        //"<span class='nav-left'>ماه بعد <i style='font-weight: bold;font-size: 9pt;' class='fas fa-arrow-left text-primary' ></i></span>"+
        ...
        "</tbody>" +
        "</table>",
        days: "<td><span class='day {{pick}} {{select}} {{today}}' data-date='{{datadate}}' onclick='SelectForActiveDate(this)' data-val='{{day}}'>{{day}}</span></td>",
