var AutomationDatepicker = function (elementID, opt) {

    // check arguments
    if (typeof elementID !== "string" || elementID.length === 0) {
        console.error("AutomationDatepicker error: input ID is not string or is empty");
        return;
    }
    ...
    options.nextButtonIcon = "/lib/AutomationDatepicker/timeir_next.png";
    options.previousButtonIcon = "/lib/AutomationDatepicker/timeir_prev.png";
    options.buttonsColor = options.buttonsColor !== undefined ? options.buttonsColor : false;
    options.forceFarsiDigits = true;
    options.markToday = true;
    options.markHolidays = options.markHolidays !== undefined ? options.markHolidays : false;
    options.highlightSelectedDay = true;
    options.sync = true;
    options.gotoToday = true;

    // create DOM
    var inputElement = $("#" + elementID);

    if ($(inputElement).val() != '') {
        $(inputElement).val(convert_to_Jalali($(inputElement).val(), "en"));
    } else {
        var toDay = new Date();
        ...
        $(inputElement).val(convert_to_Jalali(`${year}/${month}/${day}`, "en"));
    }
    ...
    $("body").append("<div id='bd-main-" + elementID + "' class='bd-main bd-hide' style='position: absolute; direction: rtl;'></div>");
    ...
    inputElement.on("focus", function () {
        ...
        var offset = $(this).offset(); // موقعیت عنصر ورودی
        var height = $(this).outerHeight(); // ارتفاع عنصر ورودی

        $("#bd-main-" + elementID).css({
            top: offset.top + height + "px", // بالا، دقیقاً زیر عنصر ورودی
            left: offset.left + "px", // چپ، هم‌راستا با عنصر ورودی
            position: "absolute", // حالت شناور
            zIndex: 9999 // نمایش تقویم بالاتر از سایر عناصر
        }).removeClass("bd-hide"); // نمایش تقویم
    ...
    $(`#bd-main-${elementID}`).on("click", "button.day", function () {
